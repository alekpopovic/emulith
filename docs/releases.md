...
Release tags use semantic versions such as `v0.1.0-poc`; prereleases do not receive a `latest` container tag. `make release-snapshot release-check` creates and validates six OS/architecture archives, SHA-256 checksums, embedded version metadata, and an SPDX JSON SBOM without publishing.
