...
| logs | CreateLogGroup | experimental | AWS-JSON-1.1 |  | Local CloudWatch Logs group/stream subset. | No IAM; throttling; retention; or full pagination semantics. | v0.2.0-dev |
| logs | PutLogEvents | experimental | AWS-JSON-1.1 |  | Basic durable event ingestion. | Sequence tokens and FilterLogEvents are limited. | v0.2.0-dev |
...
| blob | * | unsupported | REST |  | Azure endpoint foundation only. | Blob operations are not implemented. | v0.2.0-dev |
| queue | * | unsupported | REST |  | Azure endpoint foundation only. | Queue operations are not implemented. | v0.2.0-dev |
| table | * | unsupported | REST |  | Azure endpoint foundation only. | Table operations are not implemented. | v0.2.0-dev |
