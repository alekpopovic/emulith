...
CREATE TABLE IF NOT EXISTS azure_tables(account TEXT NOT NULL, name TEXT NOT NULL, created_at TIMESTAMP NOT NULL, PRIMARY KEY(account,name));
CREATE TABLE IF NOT EXISTS azure_entities(account TEXT NOT NULL, table_name TEXT NOT NULL, partition_key TEXT NOT NULL, row_key TEXT NOT NULL, etag TEXT NOT NULL, timestamp TIMESTAMP NOT NULL, properties TEXT NOT NULL, PRIMARY KEY(account,table_name,partition_key,row_key), FOREIGN KEY(account,table_name) REFERENCES azure_tables(account,name) ON DELETE CASCADE);
